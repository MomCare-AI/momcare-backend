...
def _no_auto_scoring():
    """Disconnects the post_save signal (vitals/signals.py) for this file's
    duration.

    This file tests ``reassess_risk()`` directly: create a reading via
    ``add_reading()``, then call ``reassess_risk()`` by hand and inspect what
