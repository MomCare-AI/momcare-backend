...
if env.bool("USE_DEBUG_TOOLBAR", default=True) and importlib.util.find_spec("debug_toolbar"):
    ...
    # DISABLE_PANELS only unchecks a panel — its enabled state is remembered per
    # browser, so RedirectsPanel can still switch itself back on and hold every
    # admin-action redirect on an interstitial page. Drop it from the panel list
    # entirely: redirects should just follow through.
